...
describe('Figurecon', () => {
  ...
  describe('Figurecon.get', () => {
    ...
    it('prefers global defaults instead of local', () => {
      let config = new Figurecon(resolve(__dirname, '../config.json'), {cookie: {sameSiteEnabled: false}}, {
        watch: false,
        log: false
      });
      assert.strictEqual(config.get('cookie.sameSiteEnabled', 'default'), false);
    });
